...
var servicelink="http://moshapp.kaldim.com/service/dbservice.php";
var servicelink1="http://moshapp.kaldim.com/service/dbservice.php";
...
$.mobile.changePage( "#page-chat", { transition: "slideup"} );
});
...
if(window.localStorage.getItem("taskid"))
$.mobile.changePage( "#dialog-task", { transition: "pop"} );
else
$.mobile.changePage( "#page-tasklist", { transition: "slide",reverse: false,changeHash: false} );
}
else
fadingMsg("You have not registered any game, Please contact with admin");
});
...
$.mobile.changePage( "#page-questionanswer", { transition: "slide"} );
}else{
$.mobile.changePage( "#page-scananswer", { transition: "slide",reverse: false,changeHash: false} );
}
}
else{
...
$("#flip-phone").change(function() {
 var phonesw = $(this);
    ...
    if(show){
	$(this).slider({ trackTheme: "b" });
	window.localStorage.setItem("tempphoneoption",1);
	}else{
	$(this).slider({ trackTheme: "c" });
...
$('.username').html(window.localStorage.getItem('nickname'));
else
...
var emailsw = $("#flip-email");
	$('#logout').click(function() {
			...
			//history.back();
			window.localStorage.setItem("reload");
			$.mobile.changePage( "#page-main");
...
function startTimer(){
	if(window.localStorage.getItem("gamefinish")){
		...
		}else{fadingMsg('Game has not started yet.');}
}

//show some info about current user info on main page
function ShowUserInfo(){
		if(window.localStorage.getItem("teamname"))
		$('#teamname').html(window.localStorage.getItem("teamname"));
		if(window.localStorage.getItem("taskname"))
		$('#currenttask').html(window.localStorage.getItem("taskname"));
}
	...
	$('#scan_btn').click(function() {
		 window.plugins.barcodeScanner.scan(function(result){
			 if (!result.cancelled) {
					...
					if(window.localStorage.getItem("tasksecret").toLowerCase() == scancode.toLowerCase()){
						//saves secret on localstorage for further use.
						window.localStorage.setItem('secretidfound',1);
						$.mobile.changePage( "#page-questionanswer", { transition: "slide"} );
					}else{
			    ...
			    $('#errormsg').html("Scan failed: " + error).css('color','red');
		 });
	});
});
...
isTaskSelected();
if(!window.localStorage.getItem("secretidfound"))
confirmDialog("Task","Please enter Key or scan QR Code","#page-scananswer");
...
$('#menuhide').show('fast');
		...
		//with the rest of the code below  we are retrieving all question saved in localstorage and styling view with those questions based on their question type
			for(var i=0;i<window.localStorage.getItem("numberofquestion");i++){
				...
				if(window.localStorage.getItem(("questiontype"+i))==2)
					{var aa=question.split("~&~");question=aa[0];}
			if(i===0)
	...
	for(var i=0;i<numberofimages;i++){
		...
		if(questiontype==2){
			if(choice)
			answerQuestion(choice,questionid,locationofquestion);
			else
			fadingMsg("Please select an answer");
		}else{
			if(ans)
			answerQuestion(ans,questionid,locationofquestion);
			else
			fadingMsg("Please enter answer");
		}
			});
...
function changeContent(pos){
		...
		{		
			...
			if(solved=="solved" && window.localStorage.getItem(("questionanswer"+pos))==options[i])
			$('<input data-theme="c" type="radio" name="radio-choice-1" id="radio-choice-'+i+'" value="'+options[i]+'" checked/>	<label for="radio-choice-'+i+'">'+options[i]+'</label>').appendTo('#multichoiceq');
			else
			$('<input data-theme="c" type="radio" name="radio-choice-1" id="radio-choice-'+i+'" value="'+options[i]+'" />	<label for="radio-choice-'+i+'">'+options[i]+'</label>').appendTo('#multichoiceq');
...
function answerQuestion(answer,questionid,locationofquestion){
			...
			if(data.success==1){
				...
				fadingMsg(data.error_msg);
				else
				fadingMsg("Wrong Answer!");
			}
			});
}
...
isTaskSelected();
	...
	$('#taskdictionary').click(function(){
		$.mobile.changePage( "#page-download", { transition: "slide"} );
	});
}).live('pageshow',function(event){
...
function solvedtasks(){
	...
	for(var i = 0; i< window.localStorage.getItem("numberofquestion");i++)
	{
		if(window.localStorage.getItem(("questionstatus"+i))==1)
		result++;
	}
	return result;
}
...
isTaskSelected();
	...
	if(i===0)
	$('#imgul').append('<li style="display:block"><div><img src="img/'+window.localStorage.getItem(("image"+i))+'" width="320px" height="180px"/></div></li>');
	else
	$('#imgul').append('<li style="display:none"><div><img src="img/'+window.localStorage.getItem(("image"+i))+'" width="320px" height="180px"/></div></li>');
}
