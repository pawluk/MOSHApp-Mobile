...
if(window.localStorage.getItem("scriptlat0") && window.localStorage.getItem("scriptlng0"))
tskloc=new google.maps.LatLng((window.localStorage.getItem("scriptlat0")+0.0),(window.localStorage.getItem("scriptlng0")+0.0));    
else
tskloc=new google.maps.LatLng(window.localStorage.getItem("campuslat"),window.localStorage.getItem("campuslng"));    
$('#map_canvas').gmap('addMarker', {'position': tskloc, 'icon': bimage, 
    	animation:google.maps.Animation.DROP,'shadow' : shadow, 'shape' : shadowShape, 'bounds': true   } ).click(function(e) {
...
//	                'destination' : new google.maps.LatLng(43.676082, -79.410038), 'travelMode' : google.maps.DirectionsTravelMode.DRIVING},
...
function startwatching(){
  ...
  function onSuccess(position) {
	 ...
	 if(info<1)
		 ...
		 info+=" km"
      $('#map_canvas').gmap('find', 'markers', { 'property': 'tags', 'value': 'myloc' }, function(marker, isFound) {
          if ( isFound ) {
              marker.setMap(null);
          }
      });
	  $('#map_canvas').gmap('addMarker', {'tags': 'myloc','position': myloc, animation:google.maps.Animation.DROP,
		  'icon': gimage,'shadow' : shadow, 'shape' : shadowShape, 'bounds': true  } ).click(function(e) {
	        $('#map_canvas').gmap('openInfoWindow', {'content': 'You are '+info+' far from task.'}, this); ///////SET REAL CONTENT HERE
	    });
	  hideloading();
      //clearWatch();
  }
