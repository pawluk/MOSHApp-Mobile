	...
	if ( $contacts != false ) {
		...
		while ( $row = mysql_fetch_array( $contacts ) ) {
			$response['contacts'][] = array(
				...
				"email"     => $row['e_vsbl_tm'] == 1 ? $row["u_email"] : $row['e_vsbl_tm'],
				"phone"     => $row['p_vsbl_tm'] == 1 ? $row["u_phone"] : $row['p_vsbl_tm']
			);
		...
		while ( $row = mysql_fetch_array( $teams ) ) {
			if ( $i == 0 ) {
				$team = array(
					"id"         => $row["t_id"],
					"tname"      => $row["t_name"],
					"time_spent" => is_null( $row["time_spent"] ) ? 0 : $row["time_spent"]
				);
			} else {
				if ( $team["id"] == $row["t_id"] ) {
	...
	if ( $teammemberdetails != false ) {
		...
		while ( $row = mysql_fetch_array( $teammemberdetails ) ) {
			...
			$response['tasks'][] = array(
				...
				"taskstatus" => is_null( $row["status"] ) ? 0 : $row["status"],
				"time_spent" => is_null( $row["time_spent"] ) ? 0 : $row["time_spent"]
			);
			$i++;
		}
...
function getUserTaskInfo( $u_id ) {
	...
	if ( $loggeduserinfo != false ) {
				...
				if ( $i == 0 ) {
					if ( $row['status'] != null ) {
						...
						$response['questions'][] = array(
							...
							"question"       => $row["q_text"],
							"questionstatus" => $row["q_status"] ? $row["q_status"] : 0,
							"answer"         => $answer
						);
					} else {
						if ( $row['g_id'] != null ) {
		...
		$response["error"] = 1;
		$response["error_msg"] = "Error occurred on retrieving information";
		echo json_encode( $response );
	}
}
...
function getTaskDetail( $t_id ) {
	...
	if ( $taskdetail != false ) {
		...
		while ( $row = mysql_fetch_array( $taskdetail ) ) {
			...
			$count++;
			if ( $count == $number ) {
				$response['taskname']    = $row['tsk_name'];
				$response['campus']      = $row["c_name"];
				$response['campuslat']   = $row['c_lat'];
				$response['campuslng']   = $row["c_lng"];
				$response['numberofdic'] = $number ? $number : 0;
				$response['questions']   = $sumquest;
			}
		}
		echo json_encode( $response );
	} else {
